...
function setBackgroundBoxes()  -- Set transparent black boxes
 if mode[index] == "Capture" or mode[index] == "Breeding" or mode[index] == "C-Gear" or mode[index] == "Pokemon Info" then
  gui.box(1, 13, 156, 132, "#0000007F", "#0000007F")
 ...
 if mode[index] ~= "None" then
  gui.box(152, 180, 254, 190, "#0000007F", "#0000007F")
  gui.box(193, 157, 254, 178, "#0000007F", "#0000007F")
 end
end
...
function showDateTime()
 if mode[index] ~= "None" then
  gui.box(193, 35, 254, 55, "#0000007F", "#0000007F")
  gui.text(194, 36, string.format("20%s/%s/%s", dateTime["year"], dateTime["month"], dateTime["day"]))
  gui.text(194, 47, string.format("%s:%s:%s", dateTime["hour"], dateTime["minute"], dateTime["second"]))
 end
end
...
function showCGearSeedInfo(delay)
 if showCGearSeedInfoText then
  gui.box(1, -191, 128, -106, "#0000007F", "#0000007F")
  gui.text(2, -189, string.format("Next Seed: %08X", buildSeedFromDelay(delay + 308, true)))
  gui.text(2, -178, string.format("Next Delay: %d", delay + 308))
  gui.text(2, -167, string.format("Delay: %d", delay))
  gui.text(2, -156, string.format("C-Gear Seed: %08X", cgearSeed))
  gui.text(2, -145, string.format("Hit Delay: %d", hitDelay))
  gui.text(2, -134, string.format("Hit Date/Hour:\n%s", hitDate))
 end
end
...
function showRngInfo()
 ...
 if showRngInfoText and mode[index] ~= "None" then
  gui.box(1, 135, 182, 177, "#0000007F", "#0000007F")
  gui.text(2, 136, string.format("Initial Seed: %08X%08X", initialSeedHigh, initialSeedLow))
  gui.text(2, 147, string.format("Current Seed: %08X%08X", currentSeedHigh, currentSeedLow))
  gui.text(2, 158, string.format("LCRNG Advances: %d", advances))
  gui.text(3, 169, string.format("MT Advances: %d", mtAdvances))
...
function showTrainerIDs()
 ...
 gui.text(194, 159, string.format("TID: %d", trainerTID))
 gui.text(194, 170, string.format("SID: %d", trainerSID))
end
...
function getSlotInput()
 ...
 prevKeySlot = key
 gui.box(205, 0, 254, 22, "#0000007F", "#0000007F")
 drawArrowLeft(206, 12, leftSlotArrowColor)
 gui.text(215, 12, "3 - 4")
 drawArrowRight(253, 12, rightSlotArrowColor)
 gui.text(212, 2, "Slot: "..slotIndex + 1)
...
function showIVsAndHP(ivsValue)
 ...
 gui.text(2, 58, "IVs:")
 gui.text(32, 58, string.format("%02d", hpIV), getIVColor(hpIV))
 gui.text(44, 58, "/")
 gui.text(50, 58, string.format("%02d", atkIV), getIVColor(atkIV))
 gui.text(62, 58, "/")
 gui.text(68, 58, string.format("%02d", defIV), getIVColor(defIV))
 gui.text(80, 58, "/")
 gui.text(86, 58, string.format("%02d", spAtkIV), getIVColor(spAtkIV))
 gui.text(98, 58, "/")
 gui.text(104, 58, string.format("%02d", spDefIV), getIVColor(spDefIV))
 gui.text(116, 58, "/")
 gui.text(122, 58, string.format("%02d", spdIV), getIVColor(spdIV))

 gui.text(2, 69, "HPower: "..HPTypeNamesList[hpType + 1].." "..hpPower)
end

function showMoves(moveIndexesList)
 gui.text(3, 94, "Move: "..moveNamesList[moveIndexesList[1] > 560 and 1 or moveIndexesList[1]])
 gui.text(3, 104, "Move: "..moveNamesList[moveIndexesList[2] > 560 and 1 or moveIndexesList[2]])
 gui.text(3, 114, "Move: "..moveNamesList[moveIndexesList[3] > 560 and 1 or moveIndexesList[3]])
 gui.text(3, 124, "Move: "..moveNamesList[moveIndexesList[4] > 560 and 1 or moveIndexesList[4]])
end

function showPP(movePPList)
 gui.text(120, 94, "PP: "..(movePPList[1] < 100 and movePPList[1] or 0))
 gui.text(120, 104, "PP: "..(movePPList[2] < 100 and movePPList[2] or 0))
 gui.text(120, 114, "PP: "..(movePPList[3] < 100 and movePPList[3] or 0))
 gui.text(120, 124, "PP: "..(movePPList[4] < 100 and movePPList[4] or 0))
end
...
function showInfo(pidAddr)
 ...
 if mode[index] ~= "Breeding" or isEgg then
  gui.text(2, 14, "Species: "..speciesNamesList[(speciesDexIndex > 649 or speciesDexIndex < 1) and 1 or speciesDexIndex])
  gui.text(2, 25, "PID:")
  gui.text(32, 25, string.format("%08X%s", pokemonPID, shinyType), shinyTypeTextColor)
  gui.text(2, 36, "Nature: "..natureNamesList[(natureIndex > 25 or natureIndex == nil) and 1 or natureIndex])
  gui.text(2, 47, string.format("Ability: %s (%s)", abilityNamesList[(abilityIndex > 164 or abilityIndex < 1) and 1 or abilityIndex],
  ...
  gui.text(2, 80, "Held item: "..itemNamesList[(heldItemIndex > 639) and 1 or heldItemIndex])
...
function showRoamerInfo(roamerAddr)
 ...
 if isRoamerActive then
  gui.text(2, 14, "Active Roamer? Yes")
  gui.text(2, 25, "Species: "..speciesNamesList[roamerSpeciesIndex])
  gui.text(2, 36, "PID:")
  gui.text(32, 36, string.format("%08X%s", roamerPID, roamerShinyType), roamerShinyTypeTextColor)
  gui.text(2, 47, "Nature: "..natureNamesList[roamerNatureIndex])
  showIVsAndHP(roamerIVsValue)
  gui.text(2, 79, "Level: "..roamerLevel)
  gui.text(2, 90, "HP: "..roamerHP)
  gui.text(2, 101, "Status condition: "..roamerStatus)
  gui.text(2, 112, "Current position:")
  gui.text(2, 123, (roamerMapIndex > 427 or roamerMapIndex < 1) and "" or locationNamesList[roamerMapIndex + 1],
                    roamerMapIndex == playerMapIndex and "green" or nil)
 else
  gui.text(2, 14, "Active Roamer? No")
 end
end
...
function getInfoInput()
 ...
 prevKeyInfo = key
 gui.box(188, 0, 254, 33, "#0000007F", "#0000007F")
 gui.text(189, 2, "Info Mode: ")
 gui.text(189, 13, infoMode[infoIndex])
 drawArrowLeft(207, 24, leftInfoArrowColor)
 gui.text(215, 24, "3 - 4")
 drawArrowRight(253, 24, rightInfoArrowColor)
end
...
function createStateFile(statesFileName, stateSlot)
 ...
 if statesFile then  -- Check if the state file has been created correctly
  ...
  statesFile:close()
  gui.text(2, -10, string.format("Saved state on slot %s", stateSlot))
 end
end
...
function setSaveStateValues(statesFileName, stateSlot)
 ...
 if statesFile then
  ...
  gui.text(2, -10, string.format("Loaded State %s", stateSlot))
 end
end
