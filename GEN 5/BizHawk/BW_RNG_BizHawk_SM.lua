-- Original Lua by Real96. DrawBox & PrintText tweaks by SexyMalasada.
...
if gameLanguageCode == 0x44 then  -- Check game language and set addresses
 gameLanguage = "GER"
 mtSeedAddr = 0x02215294 + getGameAddrOffset(0x20)
 mtIndexAddr = 0x02215C54 + getGameAddrOffset(0x20)
 currentSeedAddr = 0x02216164 + getGameAddrOffset(0x20)
 boxAddr = 0x0221BEEC + getGameAddrOffset(0x20)
...
elseif gameLanguageCode == 0x46 then
 gameLanguage = "FRE"
 mtSeedAddr = 0x022152D4 + getGameAddrOffset(0x20)
 mtIndexAddr = 0x02215C94 + getGameAddrOffset(0x20)
 currentSeedAddr = 0x022161A4 + getGameAddrOffset(0x20)
 boxAddr = 0x0221BF2C + getGameAddrOffset(0x20)
...
elseif gameLanguageCode == 0x49 then
 gameLanguage = "ITA"
 mtSeedAddr = 0x02215254 + getGameAddrOffset(0x20)
 mtIndexAddr = 0x02215C14 + getGameAddrOffset(0x20)
 currentSeedAddr = 0x02216124 + getGameAddrOffset(0x20)
 boxAddr = 0x0221BEAC + getGameAddrOffset(0x20)
...
elseif gameLanguageCode == 0x4A then
 gameLanguage = "JPN"
 mtSeedAddr = 0x022151B4 + getGameAddrOffset(0x20)
 mtIndexAddr = 0x02215B74 + getGameAddrOffset(0x20)
 currentSeedAddr = 0x02216084 + getGameAddrOffset(0x20)
 boxAddr = 0x0221BE0C + getGameAddrOffset(0x20)
...
elseif gameLanguageCode == 0x4B then
 ...
 mtIndexAddr = 0x02216414
 currentSeedAddr = 0x02216924
 boxAddr = 0x0221C6AC
...
elseif gameLanguageCode == 0x4F then
 gameLanguage = "USA"
 mtSeedAddr = 0x02215354 + getGameAddrOffset(0x20)
 mtIndexAddr = 0x02215D14 + getGameAddrOffset(0x20)
 currentSeedAddr = 0x02216224 + getGameAddrOffset(0x20)
 boxAddr = 0x0221BFAC + getGameAddrOffset(0x20)
...
elseif gameLanguageCode == 0x53 then
 ...
 mtIndexAddr = 0x02215CD4
 currentSeedAddr = 0x022161E4
 boxAddr = 0x0221BF6C
...
function setBackgroundBoxes()  -- Set transparent black boxes
 ...
 gui.defaultPixelFont("gens")
  ...
  gui.drawBox(1, 192, 146, 200, 0x7F000000, 0x7F000000)
 if mode[index] == "Capture" or mode[index] == "Breeding" or mode[index] == "C-Gear" or mode[index] == "Pokemon Info" then
  gui.drawBox(1, 202, 130, 311, 0x7F000000, 0x7F000000)
 elseif mode[index] == "Roamer" then
  gui.drawBox(1, 202, 130, 301, 0x7F000000, 0x7F000000)
 end

 if mode[index] ~= "None" then
  gui.drawBox(104, 376, 190, 383, 0x7F000000, 0x7F000000)
  gui.drawBox(202, 367, 254, 383, 0x7F000000, 0x7F000000)
 end
end
...
function getTabInput()
 ...
 prevKey = key
 gui.text(5, 384, "Mode: "..mode[index])
 drawArrowLeft(99, 192, leftArrowColor)
 gui.text(218, 384, "1 - 2")
 drawArrowRight(144, 192, rightArrowColor)
end
...
function showDateTime()
 if mode[index] ~= "None" then
  gui.drawBox(203, 210, 254, 226, 0x7F000000, 0x7F000000)
  gui.text(408, 420, string.format("20%s/%s/%s", dateTime["year"], dateTime["month"], dateTime["day"]))
  gui.text(428, 438, string.format("%s:%s:%s", dateTime["hour"], dateTime["minute"], dateTime["second"]))
 end
end
...
function getInitialSeedInfoInput()
 ...
 gui.drawBox(1, 376, 78, 383, 0x7F000000, 0x7F000000)
 gui.text(4, 752, showCGearSeedInfoText and "7 - Hide C-Gear" or "8 - Show C-Gear")
end

function showCGearSeedInfo(delay)
 if showCGearSeedInfoText then
  gui.drawBox(147, 238, 254, 306, 0x7F000000, 0x7F000000)
  gui.text(298, 476, string.format("Next Seed: %08X", buildSeedFromDelay(delay + 349, true)))
  gui.text(298, 494, string.format("Next Delay: %d", delay + 349))
  gui.text(298, 512, string.format("Delay: %d", delay))
  gui.text(298, 530, string.format("C-Gear Seed: %08X", cgearSeed))
  gui.text(298, 548, string.format("Hit Delay: %d", hitDelay))
  gui.text(298, 566, string.format("Hit Date/Hour:\n%s", hitDate))
 end
end
...
function showRngInfo()
 ...
 if showRngInfoText and mode[index] ~= "None" then
  gui.drawBox(1, 313, 152, 348, 0x7F000000, 0x7F000000)
  gui.text(5, 626, string.format("Initial Seed: %08X%08X", initialSeedHigh, initialSeedLow))
  gui.text(5, 644, string.format("Current Seed: %08X%08X", currentSeedHigh, currentSeedLow))
  gui.text(5, 662, string.format("LCRNG Advances: %d", advances))
  gui.text(5, 680, string.format("MT Advances: %d", mtAdvances))
...
function getRngInfoInput()
 ...
 gui.text(210, 752, showRngInfoText and "5 - Hide RNG Info" or "6 - Show RNG Info")
end
...
function showTrainerIDs()
 ...
 gui.text(408, 734, string.format("TID: %d", trainerTID))
 gui.text(408, 752, string.format("SID: %d", trainerSID))
end
...
function getSlotInput()
 ...
 prevKeySlot = key
 gui.drawBox(167, 192, 254, 200, 0x7F000000, 0x7F000000)
 drawArrowLeft(169, 192, leftSlotArrowColor)
 gui.text(356, 384, "3 - 4")
 drawArrowRight(213, 192, rightSlotArrowColor)
 gui.text(438, 384, "Slot: "..slotIndex + 1)
...
function showIVsAndHP(ivsValue)
 ...
 gui.text(4, 476, "IVs:")
 gui.text(50, 476, string.format("%02d", hpIV), getIVColor(hpIV))
 gui.text(70, 476, "/")
 gui.text(80, 476, string.format("%02d", atkIV), getIVColor(atkIV))
 gui.text(100, 476, "/")
 gui.text(110, 476, string.format("%02d", defIV), getIVColor(defIV))
 gui.text(130, 476, "/")
 gui.text(140, 476, string.format("%02d", spAtkIV), getIVColor(spAtkIV))
 gui.text(160, 476, "/")
 gui.text(170, 476, string.format("%02d", spDefIV), getIVColor(spDefIV))
 gui.text(190, 476, "/")
 gui.text(200, 476, string.format("%02d", spdIV), getIVColor(spdIV))

 gui.text(5, 494, "HPower: "..HPTypeNamesList[hpType + 1].." "..hpPower)
end

function showMoves(moveIndexesList)
 gui.text(5, 551, "Move: "..moveNamesList[moveIndexesList[1] > 560 and 1 or moveIndexesList[1]])
 gui.text(5, 569, "Move: "..moveNamesList[moveIndexesList[2] > 560 and 1 or moveIndexesList[2]])
 gui.text(5, 587, "Move: "..moveNamesList[moveIndexesList[3] > 560 and 1 or moveIndexesList[3]])
 gui.text(5, 605, "Move: "..moveNamesList[moveIndexesList[4] > 560 and 1 or moveIndexesList[4]])
end

function showPP(movePPList)
 gui.text(200, 551, "PP: "..(movePPList[1] < 100 and movePPList[1] or 0))
 gui.text(200, 569, "PP: "..(movePPList[2] < 100 and movePPList[2] or 0))
 gui.text(200, 587, "PP: "..(movePPList[3] < 100 and movePPList[3] or 0))
 gui.text(200, 605, "PP: "..(movePPList[4] < 100 and movePPList[4] or 0))
end
...
function showInfo(pidAddr)
 ...
 if mode[index] ~= "Breeding" or isEgg then
  gui.text(5, 404, "Species: "..speciesNamesList[(speciesDexIndex > 649 or speciesDexIndex < 1) and 1 or speciesDexIndex])
  gui.text(5, 422, "PID:")
  gui.text(53, 422, string.format("%08X%s", pokemonPID, shinyType), shinyTypeTextColor)
  gui.text(5, 440, "Nature: "..natureNamesList[(natureIndex > 25 or natureIndex == nil) and 1 or natureIndex])
  gui.text(5, 458, string.format("Ability: %s (%s)", abilityNamesList[(abilityIndex > 164 or abilityIndex < 1) and 1 or abilityIndex],
  ...
  gui.text(5, 512, "Held item: "..itemNamesList[(heldItemIndex > 639) and 1 or heldItemIndex])
...
function showRoamerInfo(roamerAddr)
 ...
 if isRoamerActive then
  ...
  gui.text(5, 422, "Species: "..speciesNamesList[roamerSpeciesIndex])
  gui.text(5, 440, "PID:")
  gui.text(53, 440, string.format("%08X%s", roamerPID, roamerShinyType), roamerShinyTypeTextColor)
  gui.text(5, 458, "Nature: "..natureNamesList[roamerNatureIndex])
  showIVsAndHP(roamerIVsValue)
  gui.text(5, 512, "Level: "..roamerLevel)
  gui.text(5, 530, "HP: "..roamerHP)
  gui.text(5, 548, "Status condition: "..roamerStatus)
  gui.text(5, 566, "Current position:")
  gui.text(5, 584, (roamerMapIndex > 427 or roamerMapIndex < 1) and "" or locationNamesList[roamerMapIndex + 1],
                        roamerMapIndex == playerMapIndex and "limegreen" or nil)
 else
  gui.text(5, 404, "Active Roamer? No")
 end
end
...
function getInfoInput()
 ...
 prevKeyInfo = key
 gui.drawBox(168, 192, 254, 208, 0x7F000000, 0x7F000000)
 gui.text(338, 384, "Info: "..infoMode[infoIndex])
 drawArrowLeft(209, 201, leftInfoArrowColor)
 gui.text(438, 402, "3 - 4")
 drawArrowRight(253, 201, rightInfoArrowColor)
end
